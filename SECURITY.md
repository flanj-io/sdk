...
Flanj reads real API traffic, which routinely carries sensitive and regulated data, and the
...
The SDK captures request and response bodies, so redaction is the security property everything else
...
- **Redaction at source.** Every captured body, inbound and outbound, regardless of edge
  ...
  metadata-only: their bodies are never read at all. Redacted records go only to a collector you
...
- **Local, zero external calls.** The floor is a pure function of its input. It never performs
  ...
  network code paths and no opt-in telemetry hooks.
- **Hardened, not hand-rolled.** Detection decisions are made by vetted validators (Luhn for
  card numbers, mod-97 for IBANs, phone metadata for numbers, a strict email grammar); the
