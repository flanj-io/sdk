...
Everything in this directory is a **byte-identical copy** of the canonical contract (`CONTRACTS.md` +
`v1/*`), which is maintained outside this repo. It is the agreed language between the
...
|---|---|---|
| `CONTRACTS.md` | the human-readable contract — the **public half** shared by SDK and collector (§2 OTLP convention and §6 redaction floor are the SDK-relevant sections; §5 lists only the CP endpoints the collector calls). | — |
...
**flat** (same as `collector/contracts/`). So the `./v1/…` links inside
`CONTRACTS.md` resolve to *this* directory, and its `./README.md` link refers to the canonical governance
doc, which is not vendored here.

## Changing anything

Edit the canonical file, bump `schema_version` if the change is breaking, re-vendor
byte-identically to the SDK, the collector and the control plane, and make every suite green. See
`REDACTION.md` (redaction design) for the redaction rules.
