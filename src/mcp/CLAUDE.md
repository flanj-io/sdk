...
| `resolve-mcp-edge.ts` | Edge identity: streamable-HTTP → endpoint URL host[:port] through the shared external/internal heuristic; stdio → `serverInfo.name`, class **`local-process`** (additive `flanj.edge.class` value, CONTRACTS §2). `local-process` captures + redacts bodies (a local MCP process usually fronts an external API); an `internal` HTTP peer stays metadata-only. Also `integrationForHost`, the collector's rule: with no `integration` configured, each server's is derived from its edge key (`unknown-integration` when the slug is empty), so auto-instrumented servers never share a baseline. |
