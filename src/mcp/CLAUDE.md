...
| `launch-command.ts` | `flanj.mcp.server.command` for a stdio snapshot: `command` + `args` from the transport's `_serverParams` (both package lines; never `env`/`cwd`), each element floor-redacted, compact JSON, capped at 1024 bytes with a trailing `"…"` element. Specified to the byte in CONTRACTS §2; the spec pins literal vectors shared with the Python SDK. |
