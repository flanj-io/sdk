...
we wrap the Client object, never a transport (no proxying, no stdio process spawning,
...
| `auto-instrument.ts` | The auto-patch path: `patchMcpClientConstructor` (prototype trampolines that self-instrument each instance on first use; throws on a Client it cannot patch) + `patchInstalledMcpClientsSync` / `registerMcpAutoInstrumentation` (feature-detect `@modelcontextprotocol/sdk` 1.x and `@modelcontextprotocol/client` 2.x — both OPTIONAL peers; there is no import of either package anywhere in `src/`). Each package's `require` and `import` halves are patched synchronously in the preload (the `import` half by `require(esm)`), with a real `import()` only where the runtime cannot load ESM synchronously. A missing package is skipped silently; an installed half left unpatched is reported on the one-time capture warning. |
| `resolve-import-url.mts` | The package's only ES module: a synchronous `import.meta.resolve`, loaded with `require()`, so the preload can find the file an application's `import` would load. Compiled with a scoped `@ts-expect-error` (the repo compiles as CommonJS, and `tsc` refuses `import.meta` there even in a `.mts`). |
| `assemble-mcp-call.ts` | `assembleMcpCall` — funnels through the ONE shared `assembleCapturedCall`, so every floor rule (cap, content-type gate, internal-edge metadata-only, redact-then-drop, captured-value props) applies identically. Slots: method `"tools/call"`, route/target `"/<tool>"`, url `"mcp://<peer>/<tool>"`, statusCode 0 (MCP has none). Request body = arguments; response body = `structuredContent` (JSON) else joined `content[]` text (`text/plain` — the floor's text path parses-then-traverses JSON text, so a PAN inside stringified JSON is caught structurally; the shared fixture battery pins the four MCP redaction cases). |
...
| `resolve-mcp-edge.ts` | Edge identity: streamable-HTTP → endpoint URL host[:port] through the shared external/internal heuristic; stdio → `serverInfo.name`, class **`local-process`** (additive `flanj.edge.class` value, CONTRACTS §2). `local-process` captures + redacts bodies (a local MCP process usually fronts an external API); an `internal` HTTP peer stays metadata-only. The SDK carries no integration id of its own any more — the collector derives each server's integration itself from this edge key. |
| `launch-command.ts` | `flanj.mcp.server.command` for a stdio snapshot: `command` + `args` from the transport's `_serverParams` (both package lines; never `env`/`cwd`), each element floor-redacted, compact JSON, capped at 1024 bytes with a trailing `"…"` element. Specified to the byte in CONTRACTS §2; `launch-command.spec.ts` pins the literal vectors shared with the Python SDK's suite. |
...
- **Pass-through is the contract**: result object IDENTITY preserved, arguments never
  ...
  imported).
- **Redact before attach/emit** — same day-one rule as HTTP. The four MCP redaction cases live
...
`instrumentMcpClient(client, { endpoint?, logger?, onCapture?, onSnapshot? })` — pass an
