...
describe("README — first-run essentials", () => {
  ...
  it("sends the reader to both collector run blocks, not the repo root, for the run commands", () => {
    ...
    expect(
      quickStart,
      "the retired laptop anchor must not return",
    ).not.toContain("#run-it-on-a-laptop");
  ...
  it("explains recovery for both the Docker and Kubernetes verify paths, without stale sidecar wording", () => {
    const verify = readme.slice(
      readme.indexOf("**Verify**"),
      readme.indexOf("### Configuration"),
    );
    expect(verify, "no Verify section found").not.toHaveLength(0);
    expect(verify.toLowerCase(), "sidecar wording is retired").not.toContain(
      "sidecar",
    );
  ...
  it("states the supported Node range byte-for-byte from package.json engines", () => {
    ...
    expect(
      readme,
      `README does not carry the engines range ${range}`,
    ).toContain(`Node \`${range}\``);
  });
  ...
  it("documents coexistence with OpenTelemetry, in either registration order", () => {
    const section = readme.slice(
      readme.indexOf("### Running next to OpenTelemetry"),
    );
   ...
   * startup banner prints, and the batch is silently dropped. A reader who used the
   * documented preload exactly as shown sees nothing wrong.
   */
  it("warns that process.exit() skips the beforeExit flush, with both fixes", () => {
    ...
    expect(
      section.toLowerCase(),
      "does not say beforeExit is skipped",
    ).toContain("skips");
    expect(section, "does not offer the self-started fix").toContain(
      "flanj.shutdown()",
    );
  });
   ...
   * thorough had no way to know what it does .
   */
  it("documents serverKind — values, meaning and default — next to refetchOnListChanged", () => {
    const start = readme.indexOf(
      "`start()` takes the same settings as options",
    );
    expect(
      start,
      'no "start() takes the same settings" paragraph found',
    ).toBeGreaterThan(-1);
    const paragraph = readme.slice(start, readme.indexOf("\n\n", start));
    expect(
      paragraph,
      "paragraph does not pair serverKind with refetchOnListChanged",
    ).toContain("`refetchOnListChanged`");
...
describe("README — the shared cross-language skeleton", () => {
  ...
  it('gives MCP a quick start of its own, before "What is captured"', () => {
    ...
    const section = readme.slice(
      mcpQuickStart,
      readme.indexOf("### Instrumenting a client yourself"),
    );
    ...
    for (const fact of [
      ...
      expect(section, `the MCP quick start does not mention ${fact}`).toContain(
        fact,
      );
    }
